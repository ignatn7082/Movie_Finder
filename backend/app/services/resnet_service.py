...
from app.services.base_service import DATA_DIR, get_actor_movies
# import logging # ĐÃ GỠ BỎ

# logger = logging.getLogger("app.resnet") # ĐÃ GỠ BỎ
...
# Tên file model ResNet50 dùng cho trích xuất đặc trưng nội dung
# MODEL_FILENAME = "resnet50_feature_extractor_cosine.h5" 
MODEL_FILENAME = "resnet50_feature_extractor_3.h5"

# File Index và Labels riêng cho đặc trưng NỘI DUNG (FRAME)
# CONTENT_INDEX_FILENAME = "movie_resnet50_content.index" 
# CONTENT_LABELS_FILENAME = "movie_resnet50_content_labels.npy"

CONTENT_INDEX_FILENAME = "actor_resnet50_face_3.index" 
CONTENT_LABELS_FILENAME = "actor_resnet50_face_labels_3.npy"
...
def load_resnet50_content_resources():
    ...
    if RESNET_MODEL is None and os.path.exists(RESNET_MODEL_PATH):
        try:
            print(" Loading Keras ResNet50 model...")
            RESNET_MODEL = load_model(RESNET_MODEL_PATH, compile=False) 
            
        except Exception as e:
            # Thay thế logger.error bằng print
            print(f" Cannot load Keras model from {RESNET_MODEL_PATH}: {e}")
            RESNET_MODEL = None
    ...
    if os.path.exists(CONTENT_INDEX_PATH):
        try:
            ...
            # Thay thế logger.error bằng print
            print(f"Cannot load FAISS CONTENT index: {e}")
            CONTENT_INDEX = None
...
try:
    ...
    feature_extractor_model = Model(inputs=base_model.input, outputs=base_model.layers[-2].output)
    # Thay thế logger.info bằng print
    print("[ResNet] Feature Extractor Model Loaded.")
except Exception as e:
    # Thay thế logger.error bằng print
    print(f"[ERROR] Could not load ResNet feature extractor model from {MODEL_PATH}: {e}")
    feature_extractor_model = None
...
try:
    if os.path.exists(CONTENT_INDEX_PATH):
        ...
        # Thay thế logger.info bằng print
        print(f"[ResNet] Actor Index Loaded with {CONTENT_INDEX.ntotal} vectors and {len(CONTENT_LABELS)} labels.")
    else:
        # Thay thế logger.warning bằng print
        print(f"[WARNING] ResNet] Actor Index file not found at {CONTENT_INDEX_PATH}")

except Exception as e:
    # Thay thế logger.error bằng print
    print(f"[ERROR] Could not load ResNet actor index: {e}")
    CONTENT_INDEX = None
    CONTENT_LABELS = []
...
def extract_resnet_feature(img_path):
    ...
    if feature_extractor_model is None:
        # Thay thế logger.error bằng print
        print("feature_extractor_model is not loaded.")
        return None
